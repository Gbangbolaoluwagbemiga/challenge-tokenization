...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        require(msg.value > 0 && tokens > 0, "DEX: init - must provide both ETH and tokens");
    ...
    function price(uint256 xInput, uint256 xReserves, uint256 yReserves) public pure returns (uint256 yOutput) {
        require(xReserves > 0 && yReserves > 0, "DEX: price - insufficient reserves");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        require(msg.value > 0, "DEX: deposit - must send ETH");
        ...
        if (totalLiquidity == 0) {
            ...
            tokensDeposited = msg.value; // 1:1 ratio for initial deposit
            liquidityMinted = msg.value;
        } else {
            // Calculate tokens needed based on current ratio
            tokensDeposited = (msg.value * tokenReserve) / ethReserve;
            // Calculate liquidity to mint based on proportion
            liquidityMinted = (msg.value * totalLiquidity) / ethReserve;
        }
        
        require(token.transferFrom(msg.sender, address(this), tokensDeposited), "DEX: deposit - transfer failed");
